...
import "./libs/interface/IMETAERC20.sol";
...
contract MultyTreasury is multyOwnable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SMETA }
    ...
    constructor (
        ...
        isLiquidityToken[ _METABUSD ] = true;
        liquidityTokens.push( _METABUSD );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for META
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint META needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( META ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn META for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IMETAERC20( META ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sMETA ).balanceOf( msg.sender ); // Can only borrow against sMETA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with META
        @param _amount uint
     */
    function repayDebtWithMETA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IMETAERC20( META ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, META, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( META ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( META ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns META valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match META decimals
            value_ = _amount.mul( 10 ** IERC20( META ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SMETA ) { // 9
            sMETAQueue = block.number.add( blocksNeededForQueue );
        } else return false;
